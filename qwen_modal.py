# Modal deployment for Qwen2.5-0.5B-Instruct with vLLM
...
# Qwen2.5-0.5B-Instruct - small instruction-tuned model
...
app = modal.App("qwen25-05b-inference")
...
@app.function(
    image=vllm_image,
    gpu="T4",  # T4 is sufficient for 0.5B model
...
async def test(test_timeout=10 * MINUTES):
    """Test the Qwen2.5-0.5B-Instruct endpoint."""
    import aiohttp

    url = serve.get_web_url()
    print(f"Qwen2.5-0.5B-Instruct server URL: {url}")
...
if __name__ == "__main__":
    print("Deploy with: modal deploy qwen_modal.py")
    print("Test with: modal run qwen_modal.py")
