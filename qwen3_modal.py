# Modal deployment for Qwen3-0.6B with vLLM
# OpenAI-compatible API with greedy decoding
# Using L4 GPU for faster inference
...
# Qwen3-0.6B - newer small instruction-tuned model
...
app = modal.App("qwen3-06b-inference")
...
@app.function(
    image=vllm_image,
    gpu="H100:1",  # H100 for faster inference (better than T4)
...
def serve():
    ...
    cmd = [
        ...
        "--port",
        str(VLLM_PORT),
        # Qwen3-0.6B settings (supports up to 32768, using 8192 for input+output headroom)
        "--max-model-len", "8192",
        "--dtype", "float16",
    ]
...
async def test(test_timeout=10 * MINUTES):
    """Test the Qwen3-0.6B endpoint."""
    import aiohttp

    url = serve.get_web_url()
    print(f"Qwen3-0.6B server URL: {url}")
...
Output only FOL hypotheses, one per line."""
    ...
    async with aiohttp.ClientSession(base_url=url) as session:
        ...
        async with session.post("/v1/chat/completions", json=payload, headers=headers, timeout=2 * MINUTES) as resp:
            ...
            print(f"Qwen3-0.6B response:\n{response}")


if __name__ == "__main__":
    print("Deploy with: modal deploy qwen3_modal.py")
    print("Test with: modal run qwen3_modal.py")
