"""
Fast Symbolic FOL experiment runner with parallel async requests.
Uses abstract symbols (c1, p1, e1) instead of meaningful names.
...
from symbolic_fol import convert_ontology_to_symbolic
...
- pred(const) for ground atoms (e.g., c1(e1), p1(e2))
- ∀x(P(x) → Q(x)) for universal rules (e.g., ∀x(c1(x) → p1(x)))
- Use ¬ for negation (e.g., ¬p1(e1), ∀x(c1(x) → ¬p2(x)))
Output only FOL hypotheses, one per line."""


def make_symbolic_fol_user_prompt(symbolic_theories, symbolic_observations):
    return f"Theories: {symbolic_theories} Observations: {symbolic_observations} Produce hypotheses to explain observations."
...
async def process_single_example(client, model_name, ontology, semaphore):
    """Process a single example with symbolic FOL and rate limiting via semaphore."""
    async with semaphore:
        # Convert to symbolic FOL
        sym_theories, sym_observations, sym_hypotheses, converter = convert_ontology_to_symbolic(ontology)
        ...
        # Evaluate against symbolic ground truth
        pred_hyps = parse_fol_hypotheses_from_response(reply)
        gt_hyps = parse_fol_ground_truth(sym_hypotheses)

        strong_acc = compute_fol_strong_accuracy(pred_hyps, gt_hyps)
        weak_acc = compute_fol_weak_accuracy(pred_hyps, gt_hyps, sym_observations, sym_theories)
        ...
        return {
            'reply': reply,
            'result': {'strong': strong_acc, 'weak': weak_acc},
            'mapping': converter.get_mapping(),
            'symbolic_theories': sym_theories,
            'symbolic_observations': sym_observations,
            'symbolic_hypotheses': sym_hypotheses
        }


async def run_symbolic_fol_zero_shot_single_hypothesis_async(model_name, task_type, height, num_examples=NUM_EXAMPLES, base_url=None):
    """Run zero-shot single hypothesis experiment with symbolic FOL and parallel async requests."""
    ...
    print(f"\n{'='*60}")
    print(f"Symbolic FOL Zero-shot Single Hypothesis (FAST): {task_type}, Height {height}")
    print(f"{'='*60}")
    ...
    mappings = [r['mapping'] for r in responses]
    symbolic_gts = [r['symbolic_hypotheses'] for r in responses]
    ...
    output_file = output_dir / f"symbolic_fol_zeroshot_single_{task_type}_h{height}_{sanitize_model_name(model_name)}.pkl"
    with open(output_file, 'wb') as f:
        pickle.dump({
            ...
            'results': results,
            'mappings': mappings,
            'symbolic_ground_truths': symbolic_gts,
            'metrics': {
    ...
    return {'strong_accuracy': strong_mean, 'weak_accuracy': weak_mean}


def main():
    parser = argparse.ArgumentParser(description='Run INABHYD Symbolic FOL experiments (FAST)')
    ...
    for height in heights:
        asyncio.run(run_symbolic_fol_zero_shot_single_hypothesis_async(
