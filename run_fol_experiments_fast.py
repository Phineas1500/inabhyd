"""
Fast FOL experiment runner with parallel async requests.
...
async def process_single_example(client, model_name, ontology, semaphore):
    ...
    async with semaphore:
        ...
        if NO_SYSTEM_PROMPT:
            ...
            combined_prompt = f"{FOL_SYSTEM_PROMPT}\n\n{user_prompt}"
...
async def run_fol_zero_shot_single_hypothesis_async(model_name, task_type, height, num_examples=NUM_EXAMPLES, base_url=None):
    ...
    print(f"\n{'='*60}")
    print(f"FOL Zero-shot Single Hypothesis (FAST): {task_type}, Height {height}")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description='Run INABHYD FOL experiments (FAST)')
    ...
    for height in heights:
        asyncio.run(run_fol_zero_shot_single_hypothesis_async(
